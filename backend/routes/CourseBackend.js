...
router.post('/delcourse', [body('courseRId').notEmpty().withMessage('Course RID is required')], async (req, res) => {
	// console.log(req.body);
	// Check for validation errors
	const errors = validationResult(req);
	if (!errors.isEmpty()) {
...
router.post('/regcourse', [], async (req, res) => {
...
router.post('/getBatchElecCourse', [], async (req, res) => {
	try {
		const mydata = await Course.find({ batch: req.body.batch, courseType: { $ne: 'Program Course' } })
			.populate('Faculty')
			.exec();
		// console.log(mydata);
		return res.json({ success: true, data: mydata });
