...
export default function ElectiveSelection() {
	...
	async function handleConfirm1() {
		try {
			...
			let response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/regcourse`, {
				method: 'POST',
				headers: {
	...
	useEffect(() => {
		async function fetchData() {
			try {
				const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getBatchElecCourse`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
				...
				const response1 = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getStudentData`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
			...
			<div className='scrollable-wrapper' style={{ position: 'relative', maxHeight: '70vh', overflowY: 'scroll' }}>
				<div className='scrollable-section' style={{ position: 'relative', padding: '10px' }}>
					<section className='accordion'>
						{/* Accordion 1 */}
						<div className='tab'>
							<input type='checkbox' name='accordion-1' id='cb1' checked={isChecked1} onChange={() => setIsChecked1(!isChecked1)} />
														...
														<tr key={course.id}>
															...
															<td>
																{course.Faculty.map((fac, facIndex) => (
																	<React.Fragment key={facIndex}>{fac.name}&nbsp;&nbsp;</React.Fragment>
																))}
															</td>
						...
						{/* Accordion 2 */}
						<div className='tab'>
							<input type='checkbox' name='accordion-2' id='cb2' checked={isChecked2} onChange={() => setIsChecked2(!isChecked2)} />
						...
						{/* Accordion 3 */}
						<div className='tab'>
							<input type='checkbox' name='accordion-3' id='cb3' checked={isChecked3} onChange={() => setIsChecked3(!isChecked3)} />
						...
						{/* Accordion 4 */}
						<div className='tab'>
							<input type='checkbox' name='accordion-4' id='cb4' checked={isChecked4} onChange={() => setIsChecked4(!isChecked4)} />
						...
						{/* Accordion 5 */}
						<div className='tab'>
							<input type='checkbox' name='accordion-5' id='cb5' checked={isChecked5} onChange={() => setIsChecked5(!isChecked5)} />
