...
export default function AdminMain() {
	...
	const handleCourseRegClick = async () => {
		...
		try {
			const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/updateswitches`, {
				method: 'POST',
				headers: {
	...
	const AccordionBatch = text => {
		return (
			<div className='tab'>
				<input type='checkbox' name={`accordion-${text}`} id={`cb${text}`} />
									...
									if (course.batch === text) {
										return (
											<tr key={course.id}>
												...
												<td>
													<button onClick={handleCourseInfo(course._id)}>{course.name}</button>
												</td>
												<td>
	...
	useEffect(() => {
		...
		async function fetchData() {
			console.log(`${process.env.REACT_APP_BACKEND_URL}/getAllCourses`);
			try {
				const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getAllCourses`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
				...
				const response1 = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getswitches`, {
					method: 'GET',
					headers: {
						'Content-Type': 'application/json',
					},
	...
	return (
		<div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
			<div className='button-container' style={{ position: 'sticky', top: 0, zIndex: 1, padding: '10px', marginLeft: 'auto', marginRight: 'none' }}>
				<div className='buttonHolder' style={{ width: '10%' }}>
			...
			<div className='button-container' style={{ position: 'sticky', top: 0, zIndex: 1, padding: '10px' }}>
				<div className='buttonHolder'>
					<Link to='/generatett'>
						<button className='button'>Generate Timetable</button>
					</Link>
				</div>
				<div className='buttonHolder'>
			...
			<div className='button-container' style={{ position: 'sticky', top: 0, zIndex: 1, padding: '10px' }}>
				...
				<div className='buttonHolder'>
					{Sdata.CourseReg && (
						<button className='button' onClick={handleCourseRegClick}>
							{' '}
							Disable Course Registration
						</button>
					)}
					{!Sdata.CourseReg && (
						<button className='button' onClick={handleCourseRegClick}>
				...
				<div className='buttonHolder'>
					{Sdata.CourseDrop && (
						<button className='button' onClick={handleCourseDropClick}>
							Disable Course Drop
						</button>
					)}
					{!Sdata.CourseDrop && (
			...
			<div className='button-container' style={{ position: 'sticky', top: 0, zIndex: 1, padding: '10px' }}>
				<div className='buttonHolder'>
					...
					<button className='button' onClick={handleUploadClick} style={{ width: '20%' }}>
						Upload Timetables
					</button>
				</div>
			</div>
