...
export default function CourseReg() {
	...
	const [mandatoryCoursesButton, setMandatoryCoursesButton] = useState(<button className='custom-button'>First Select All Mandatory Courses</button>);
	...
	const initializeArray = (size, ac, avc) => {
		...
		setCount(temp);
		if (temp === size)
			setMandatoryCoursesButton(
				<button className='custom-button danger' onClick={handleFinalClick}>
					Proceed To Elective Selection
				</button>
			);
	};
	...
	useEffect(() => {
		async function fetchData() {
			try {
				const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getBatchCoreCourse`, {
					method: 'POST',
					headers: {
	...
	const handleRegistration = useCallback(
		(id, index) => () => {
			async function fetchData() {
				try {
					const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/regcourse`, {
						method: 'POST',
						headers: {
							'Content-Type': 'application/json',
						},
					...
					if (!json.success) {
						...
						if (count === maxCount)
							setMandatoryCoursesButton(
								<Link to='/electiveselection'>
									<button className='custom-button danger'>Proceed To Elective Selection</button>
								</Link>
							);
					}
				} catch (error) {
	...
	return (
		<div>
			...
			<div>
				...
				<div style={{ display: 'flex', justifyContent: 'center' }}>{mandatoryCoursesButton}</div>
			</div>
		</div>
	);
}
