...
export default function ALFaculty() {
	...
	useEffect(() => {
		...
		async function fetchData() {
			try {
				const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getFacultyData`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
			...
			<div className='main-content'>
				...
				<div className='white-div'>
					...
					<div className='button-container'>
						<button className='button' style={{ width: 'auto', marginLeft: 'auto', marginRight: 'none' }} onClick={logout}>
							Log Out
						</button>
					</div>
						...
						<table>
							...
							<tbody>
								{data.Courses.map((course, index) => (
									<tr key={index}>
										<td>
											<button className='coursebuttons' onClick={handleCourseInfo(course._id)}>
												{course.name}
											</button>
										</td>
										<td>{course.batch}</td>
