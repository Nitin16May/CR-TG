...
export default function CourseView() {
	...
	async function handleAskStudents() {
		const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getStudentsOfCourse`, {
			method: 'POST',
			headers: {
	...
	return (
		<div>
				...
				{data.Faculty && data.Faculty.length > 0 ? (
					<div style={{ width: 'auto' }}>
						{data.Faculty.map((fac, index) => (
							<p key={index} style={{ textAlign: 'left', marginLeft: '30px' }}>
								{fac.name}&nbsp;&nbsp;|&nbsp;&nbsp;{fac.email}
							</p>
						))}
					</div>
			...
			<div style={{ display: 'flex', justifyContent: 'center' }}>
				<div>
					<Link to='/'>
						<button className='complete' style={{ marginRight: '30px' }}>
							Back To Home
						</button>
					</Link>
					{isAdmin && (
