...
export default function GenerateTT() {
	...
	useEffect(() => {
		...
		async function fetchData() {
			try {
				const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getCourses`, {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
	...
	const handleSubmit = async event => {
		...
		const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/gentimetable`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
										...
										return (
											<tr key={course.id}>
												<td>
													<input
														type='checkbox'
														value={`${course.name} ${course.batch} ${course.Faculty.length} ${course.Faculty.map(
															fac => fac.name
														).join(' ')}`}
