...
export default function Signup() {
	...
	const handleSubmit = async e => {
		e.preventDefault();
		if (creds.email.split('@')[1] !== process.env.REACT_APP_ALLOWED_EMAIL_DOMAIN) {
			alert(`Only Domains with '${process.env.REACT_APP_ALLOWED_EMAIL_DOMAIN}' are allowed!`);
			return true;
		}
		...
		<div style={{ width: '50%', marginLeft: 'auto', marginRight: 'auto' }}>
			<form onSubmit={handleSubmit}>
				<div className='form-group' style={{ marginBottom: '20px' }}>
					<label htmlFor='usertype' style={{ marginBottom: '5px', display: 'block' }}>
						User
					</label>
					<select
						...
						onChange={onChange}
						style={{ width: '100%', padding: '5px', borderRadius: '4px', borderColor: '#ccc' }}
					>
				...
				<div className='form-group' style={{ marginBottom: '20px' }}>
					<label htmlFor='exampleInputEmail1' style={{ marginBottom: '5px', display: 'block' }}>
						Email address
					</label>
					<input
						type='email'
				...
				<button
					...
					style={{ padding: '8px 20px', borderRadius: '4px', backgroundColor: '#007bff', color: '#fff', border: 'none' }}
				>
					Submit
				</button>
			</form>
